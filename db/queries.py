...
def get_or_create_topic(name: str, is_evolving: int = 1) -> int:
    with get_db() as conn:
        ...
        if row:
            return row["id"]
        cur.execute("INSERT INTO topics (name, is_evolving) VALUES (?, ?)", (name, is_evolving))
...
def _get_sources_for_entry(cur, entry_id: int) -> list:
    ...
    cur.execute("SELECT source_url FROM qa_entry_sources WHERE qa_entry_id = ?", (entry_id,))
...
def update_user_knowledge(topic_id: int, proficiency: int = None):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("SELECT * FROM user_knowledge WHERE topic_id = ?", (topic_id,))
        row = cur.fetchone()
        if row:
...
def save_review_summary(
    week_start: str, new_entries: int, topics_covered: int, outdated_flagged: int, digest: str
) -> int:
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(
